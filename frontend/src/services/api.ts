...
// Detect if running on Vercel or cloud static host without co-located Node server
export const isCloudPreview =
...
// Helper to determine if we should fall back to client mock
const shouldFallback = (error: any) => {
  if (isCloudPreview) return true;
...
export const authApi = {
  ...
  refresh: async () => {
    if (isCloudPreview) {
      ...
      if (!user) {
        throw new Error('No active cloud preview session');
      }
      return {
        user,
  ...
  getMe: async (): Promise<User> => {
    if (isCloudPreview) {
      return getMockUser() || mockUsers[0];
    }
    try {
